/*******************************************************************************
*                                                                              *
*   PrimeSense NITE 1.3 - Stick Figure Sample                                  *
...
void XN_CALLBACK_TYPE CalibrationCompleted(xn::SkeletonCapability& skeleton, XnUserID user, XnCalibrationStatus eStatus, void* pCookie)
{
...
int main(int argc, char **argv)
{
	...
	g_UserGenerator.RegisterToUserExit(UserExit, NULL, h);
	g_UserGenerator.RegisterToUserReEnter(UserReEnter, NULL, h);
	g_UserGenerator.GetSkeletonCap().SetSkeletonProfile(XN_SKEL_PROFILE_ALL);

	XnCallbackHandle hCalibStart, hCalibComplete;
	XnCallbackHandle hPoseDetected;
	rc = g_UserGenerator.GetSkeletonCap().RegisterToCalibrationStart(CalibrationStart, NULL, hCalibStart);
	CHECK_RC(rc, "Register to calibration start");
	rc = g_UserGenerator.GetSkeletonCap().RegisterToCalibrationComplete(CalibrationCompleted, NULL, hCalibComplete);
	CHECK_RC(rc, "Register to calibration complete");
	rc = g_UserGenerator.GetPoseDetectionCap().RegisterToPoseDetected(PoseDetected, NULL, hPoseDetected);
	...
	g_subSample = depthMD.XRes()/320;
	if (g_subSample == 0) g_subSample = 1;
	...
	glutCreateWindow("PrimeSense Stick Figure Sample");
...
void drawOrientation(XnUserID user, xn::UserGenerator userGenerator, XnSkeletonJoint joint, const XnPoint3D& corner)
{
	...
	virt2.position = xnCreatePoint3D(virt1.position.X+100*orientation.orientation.elements[0],
	...
	virt1.position = position.position;
	virt2.position = xnCreatePoint3D(virt1.position.X+100*orientation.orientation.elements[1],
	...
	virt1.position = position.position;
	virt2.position = xnCreatePoint3D(virt1.position.X+100*orientation.orientation.elements[2],
...
void draw_pointcloud()
{
	...
	for (XnUInt16 nY = 0; nY < nYRes; nY++)
	{
		if (g_subSample > 1 && nY%g_subSample!=0)
		{
			// Skip line
			pLabels += nXRes; 
			nIndex += nXRes; 
		...
		{
			if (g_subSample > 1 && nX % g_subSample != 0)
			{
				// Skip column
				continue;
			}
